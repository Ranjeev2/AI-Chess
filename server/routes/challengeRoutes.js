...
router.get('/today', ensureAuthenticated, async (req, res) => {
  try {
    ...
    if (!validateFEN(challenge.initialPosition)) {
      console.error('Invalid FEN in challenge:', challenge.initialPosition);
      // If FEN is invalid, regenerate challenge
      challenge = await generateDailyChallenge();
      
      // If still invalid, send an error response
      if (!validateFEN(challenge.initialPosition)) {
        return res.status(500).json({ 
          message: 'Error: Unable to generate a valid challenge. Please try again later.' 
        });
      }
    }
    ...
    res.json({
      _id: challenge._id,
      initialPosition: challenge.initialPosition,
